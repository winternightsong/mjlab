...
def main() -> None:
    ...
    parser.add_argument(
        "--cut", nargs=3, type=float, action="append", metavar=("LEFT", "RIGHT", "TRANSITION")
    )
    ...
    cut_cfg = tuple(Cut(*values) for values in args.cut) if args.cut else CUTS
    cuts = [(round(c.left_s * fps), round(c.right_s * fps), round(c.transition_s * fps)) for c in cut_cfg]
    ...
    for left, right, count in cuts:
        ...
        tr = transition(data, left, right, count, linear=args.linear)
        for key in dynamic:
