...
# 确保这个 config 路径在你的项目中存在，如果不一样请替换为你的 S45 env config
...
class MotionLoader:
    ...
    def _load_motion(self):
        ...
        # S45 base height is around 0.82m, adjust if your CSV data is biased
        # motion[:, 2] += 0.0  
...
def run_sim(
    ...
    # Verify joints exist in S45 model before proceeding
    ...
    while not file_saved:
        ...
        # Ensure motion data matches joint definition length
        if motion_dof_pos.shape[1] != len(robot_joint_indexes):
             # Try to slice or pad if dimensions mismatch (S45 usually has 28 or 26 dof)
             # S45: 12 legs + 14 arms + 2 head = 28 joints
             current_dim = motion_dof_pos.shape[1]
             target_dim = len(robot_joint_indexes)
             if current_dim > target_dim:
                 motion_dof_pos = motion_dof_pos[:, :target_dim]
                 motion_dof_vel = motion_dof_vel[:, :target_dim]
...
def main(
    ...
    # --- S45 Joint List Correction ---
    # Replaced G1 joints (hip_pitch/roll/yaw etc.) with S45 joints (leg_l1...l6).
    # Removed waist joints (S45 has no waist).
    # Added head joints (S45 has head).
    s45_joint_names = [
        # Left Leg (1-6)
        "leg_l1_joint",
        "leg_l2_joint",
